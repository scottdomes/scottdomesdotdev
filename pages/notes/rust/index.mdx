...
A package contains a `Cargo.toml` file. When you run `cargo new`, you get a package.

Modules are groups of code within a crate. You can define a module with the `mod` keyword.

Here's some nested modules for a restaurant app:
...
Here's the module tree created by this:
...
You can use absolute or relative paths to navigate modules.

Here's how we'd access a module function from a public method on our API (declared with `pub`):
...
Should you use absolute or relative paths? The Rust book recommends absolute paths, since your function definitions and invocations are likely to be moved independently of each other (i.e. it makes sense to update the path when you move the definition, and not when you move the invocation).

The above code won't compile, however, because of privacy! In Rust, all items are private by default, including structs, functions, modules, enums, and constants.

Children can see their parents, but parents can't see their children. 

You use the `pub` keyword to expose certain parts.

To use `hosting::add_to_waitlist`, we need make both the `hosting` module public, AND the `add_to_waitlist` function.
...
`eat_at_restaurant` can already see `front_of_house` since they're at the same level, but it can't see the _contents_ of `front_of_house` unless they are public. Similarly, we can access `hosting` but not its functions, unless they are public too.

You can use the `super` keyword to go up a level in a relative path:
...
Here's a great example of `pub` used with structs and their data:
...
The `Breakfast` struct has both private and public fields. By contrast, a public enum has all of its variants become public.
...
The idea here is that enums aren't useful unless all variants are public, whereas there are plenty of cases where structs should have a mix of public and private data.

Imagine we're using the same module + submodules over and over. It might get annoying to write its path out. We can shorten our paths with `use`:
...
You can also use relative paths like this. In this case, `use self::front_of_house::hosting;` would have the same effect.

While you _could_ do `use crate::front_of_house::hosting::add_to_waitlist` and then just invoke `add_to_waitlist()`, that's bad practice, since then `add_to_waitlist` appears to be locally defined.

The exception is with structs/enums, in which case you bring in the submodule:
...
If two modules have the same name, you just stop at the parent:
...
You can also re-export modules by using `pub use`. In the below code, code that calls `eat_at_restaurant` can now also use `hosting::add_to_waitlist`:
...
Doing so abstracts our API a bit. The consumer of `eat_at_restaurant` does not ever need to know about `front_of_house`; they just use `hosting::add_to_waitlist`. This makes sense if you use the customer-in-a-restaurant metaphor: they don't need to know about the front_of_house distinction.

Here's how you import multiple submodules:
...
Or import all public submodules:

```rust
use std::collections::*;
```

... but you probably want to avoid that.

When dividing up your module into different files, Rust can pick up the paths automatically. In your root file (either `src/lib.rs` or `src/main.rs`), the following code will automatically load `src/front_of_house.rs`:
