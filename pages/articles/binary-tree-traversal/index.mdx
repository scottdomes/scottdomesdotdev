...
5. Since the queue length is now 1, the `while` loop in `inorderTraverse` calls `traverse` again.
6. Since there is NO `currentNode`, `traverse` removes Node 1 from the queue and pushes it into output. It sets the `currentNode` to Node 1's right child, which is Node 3.
7. The queue length is now 0, but there is a `currentNode`, so the `while` loop in `inorderTraverse` calls `traverse` again.
8. Since there is a `currentNode`, `traverse` pushes Node 3 into the queue, and sets the `currentNode` to its left child... which is null. So `currentNode` is now null.
9. The queue length is now 1, so `inorderTraverse` calls `traverse` again.
10. Since there is NO `currentNode`, `traverse` removes Node 3 from the queue and pushes it into output. It sets the `currentNode` to Node 3's right child, which is null.
11. The queue length is now 0. `currentNode` is null. `inorderTraverse` returns `output`.
...
Iterative is, in some ways, the "reverse" of a preorder traversal. Instead of going node -> children, we go children -> node. However, it's not a true reversal, as we still want to go from left to right. Instead of node -> left -> right, we go left -> right -> node.

The easiest way to implement postorder is to change the way we add nodes to the `output` array. Instead of `push`ing them to the back of the array, we `unshift` them to the front. Note that `unshift` is less efficient than `push`, since all items in the array have to be shifted in memory. If you use this solution in an interview, be sure to note that fact.

The alternate way to implement a postorder traversal, avoiding `unshift`, is to use some mechanism to keep track of previous nodes visited. The trouble with postorder is going from parent to left sibling to right sibling to parent; when we return to the parent, how do we know we've already visited the left node?

These solutions are more complex so I decided to not to include them here. The `unshift` method is still very efficient and simple to implement. You can see an alternate [two-queue solution here](https://www.geeksforgeeks.org/iterative-postorder-traversal/).
...
This implementation is exactly the same as a preorder traversal, except we swap `push` for `unshift` and swap the order with which we deal with the children.
...
1. `postorderTraverse` calls `traverse` with root node in queue.
2. `traverse` takes Node 1 out of the queue and unshifts it into `output`, then pushes its left child (Node 2) and its right child (Node 3) into the queue.
3. Since the queue length is now 2, the `while` loop in `postorderTraverse` calls `traverse` again.
4. `traverse` takes Node 3 out of the queue and unshifts it into `output`. Since it has no children, nothing else happens.
5. Since the queue length is 1, the `while` loop in `postorderTraverse` calls `traverse` again.
6. `traverse` takes Node 2 out of the queue and unshifts it into `output`. Since it has no children, nothing else happens.
7. The queue length is now 0. `postorderTraverse` returns `output`.

Here's the function calls, showing what's happening in the queue:

1. `postorderTraverse` is called with the root node (Node 1).
2. `traverse([Node1])` is called from `postorderTraverse`.
3. `traverse([Node2, Node3])` is called from `postorderTraverse`.
4. `traverse([Node2])` is called from `postorderTraverse`.

In this case, it might help to see the output and queue side by side:

1. Queue: `[Node 1]`, Output: `[]`
2. Queue: `[Node2, Node3]`, Output: `[Node1]`
3. Queue: `[Node2]`, Output: `[Node3, Node1]`
4. Queue: `[]`, Output: `[Node2, Node3, Node1]`
